...
| 提示 | 原因 | 处理 |
| --- | --- | --- |
| 无法识别视频链接 | 链接不是抖音视频/图集分享链接 | 重新复制抖音 App 的分享链接 |
| Cookie 已失效，请在页面上重新扫码登录 | 登录态过期 | 在页面重新扫码登录 |
| 请求被抖音风控拦截（签名校验未通过） | 抖音升级风控，程序签名方案过时 | 更新程序版本 |
| 请求过于频繁被抖音限流 | 短时间解析太多次 | 稍等一分钟再试 |
| 请求抖音超时 / 网络请求失败 | 本机网络问题 | 检查网络后重试 |
| 抖音返回错误（status_code=…）或未返回作品数据 | 作品不存在、已删除或不可见 | 换个作品链接 |
| 当前页面只支持视频，不支持图集 | 链接是图集 | 暂不支持图集下载 |

2026-09 起抖音对网页接口启用了 Argus 风控（`x-secsdk-web-signature` 签名校验），解析请求已按对应算法签名。如果再次出现“被抖音风控拦截”，说明抖音更换了签名盐值，需要更新 `douyin_video_parser.py` 中的 `WEB_SIGN_SALT`。
